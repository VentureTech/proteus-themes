...
(function ($) {
  ...
  var noProtocolUrl = /(^|["'(\s]|&lt;)(www\..+?\..+?)((?:[:?]|\.+)?(?:\s|$)|&gt;|[)"',])/g,
      ...
      linkifier = function (html) {
        // NOTE: we escape `"http` as `"<``>` to make sure `httpOrMailtoUrl` below doesn't find it as a false-positive
      ...
      linkify = $.fn.linkify = function (cfg) {
        ...
        if (use === undefined || use === '*') // use === undefined  ||  use === null
        {
          for (var pluginName in allPlugins) {
            if (allPlugins.hasOwnProperty(pluginName))
              plugins.push(allPlugins[pluginName]);
          }
        }
        else {
          ...
          while (i--) {
            ...
            if (n.nodeType === 3) {
              ...
              if (html.length > 1 && /\S/.test(html)) {
                  ...
                  while (k--) {
                    ...
                    if (tmpNode.nodeType === 3) {
                      ...
                      if (html.length > 1 && /\S/.test(html)) {
                        ...
                        html = $.isFunction(plugin) ? plugin(html) : html.replace(plugin.re, plugin.tmpl);
                        htmlChanged = htmlChanged || preHtml !== html;
                        if (preHtml !== html)  $(tmpNode).after(html).remove();
                      }
                    }
                  }
                }
                ...
                if (htmlChanged) $(n).after(html).remove();
              }
            }
        ...
        if (callback) callback($(newLinks.reverse()));
        return this;
      };
  /*
   linkify.plugins = {
